...
from cera import app_settings as settings
...
def read_array(filename, varname, when):
    ...
    try:
       ...
       log.error('Failed to load file for {varname} at {when} in file {filename}'.format(varname=varname,when=when,filename=filename))
       log.error(e)
